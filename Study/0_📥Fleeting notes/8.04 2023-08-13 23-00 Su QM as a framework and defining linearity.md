...
Maxwell is linear theory. Linearity In practice:
...
## 2.2 It's very useful in practice. Why?
...
$$LU = 0 \tag{1}$$
where $L \equiv$ Linear operator and $U \equiv$ Unknown. In order to check linearity, it's just enough to check two below conditions
$$L(aU) = a LU \;\;and \;\;\; L(U_{1}+LU_{2}) = LU_{1}+U_{2}$$
